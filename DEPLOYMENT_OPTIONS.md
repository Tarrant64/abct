...
```bash
./abct-docker/update-unraid.sh YOUR_SERVER_IP 8081
```
...
```bash
./abct-docker/clean-deploy-unraid.sh YOUR_SERVER_IP 8081
```
...
```bash
./abct-docker/deploy-from-git.sh YOUR_SERVER_IP 8081 [branch]
```
...
**Example with branch**:
```bash
./abct-docker/deploy-from-git.sh YOUR_SERVER_IP 8081 feature/new-dashboard
```
...
./abct-docker/deploy-from-git.sh YOUR_SERVER_IP 8081
```
...
### Check Status
```bash
ssh root@YOUR_SERVER_IP "docker logs abct-dashboard --tail 50"
```

### Verify Version
```bash
curl -s http://YOUR_SERVER_IP:8081/ | grep BUILD
```
...
### Access Container
```bash
ssh root@YOUR_SERVER_IP "docker exec -it abct-dashboard bash"
```

### View Database
```bash
ssh root@YOUR_SERVER_IP "docker exec abct-dashboard sqlite3 /app/data/portfolio.db 'SELECT * FROM users;'"
```
...
# Test on http://YOUR_SERVER_IP:8081

# If issues, rollback to previous commit
git log --oneline  # Find previous commit hash
./abct-docker/deploy-from-git.sh YOUR_SERVER_IP 8081 <commit-hash>
```
