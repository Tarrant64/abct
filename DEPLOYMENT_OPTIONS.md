...
```bash
./abct-docker/clean-deploy-unraid.sh 192.168.1.100 8081
```
...
```bash
./abct-docker/deploy-from-git.sh 192.168.1.100 8081 [branch]
```
...
**Example with branch**:
```bash
./abct-docker/deploy-from-git.sh 192.168.1.100 8081 feature/new-dashboard
```
...
./abct-docker/deploy-from-git.sh 192.168.1.100 8081
```
...
### Check Status
```bash
ssh root@192.168.1.100 "docker logs abct-dashboard --tail 50"
```

### Verify Version
```bash
curl -s http://192.168.1.100:8081/ | grep BUILD
```
...
### Access Container
```bash
ssh root@192.168.1.100 "docker exec -it abct-dashboard bash"
```

### View Database
```bash
ssh root@192.168.1.100 "docker exec abct-dashboard sqlite3 /app/data/portfolio.db 'SELECT * FROM users;'"
```
...
# Test on http://192.168.1.100:8081

# If issues, rollback to previous commit
git log --oneline  # Find previous commit hash
./abct-docker/deploy-from-git.sh 192.168.1.100 8081 <commit-hash>
```
