...
async def init_db():
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        # ABCT-CARDANO-ACCOUNT-LEVEL-20260919: stake_address groups payment
        # addresses that belong to the same Cardano stake key/account (a
        # hardware wallet reports the whole account, not one address -- see
        # the task notes). Resolved lazily and cached forever once set (a
        # payment address's stake credential never changes). label_is_auto
        # marks a label this app generated itself (e.g. stake rediscovery's
        # "Discovered (...)") vs one the user typed, so canonical-wallet
        # selection within a stake group can prefer the user's own label.
        try:
            await db.execute("ALTER TABLE wallets ADD COLUMN stake_address TEXT")
        except Exception:
            pass  # Column already exists
        try:
            await db.execute("ALTER TABLE wallets ADD COLUMN label_is_auto INTEGER DEFAULT 0")
        except Exception:
            pass  # Column already exists
        await db.execute("""
            CREATE INDEX IF NOT EXISTS idx_wallets_stake_address
            ON wallets(user_id, stake_address)
        """)
...
async def save_wallet(address: str, blockchain: str, label: str = None, user_id: int = None,
    ...
    Args:
        ...
        is_auto_label: True if `label` was generated by ABCT itself (e.g.
            stake-key rediscovery's "Discovered (...)") rather than typed by
            the user -- used by canonical-wallet-per-stake-key selection
            (ABCT-CARDANO-ACCOUNT-LEVEL-20260919) to prefer a user's own
            label over an auto-generated one. Only applied on INSERT; an
            existing wallet's label_is_auto is left as whatever it already
            was, since save_wallet() is also used for plain re-labeling.
    """
    if user_id is None:
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        if existing:
            ...
            # Insert new wallet
            await db.execute(
                "INSERT INTO wallets (user_id, address, blockchain, label, label_is_auto, updated_at) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (user_id, address, blockchain, label, 1 if is_auto_label else 0, datetime.now())
            )
        await db.commit()
...
async def set_wallet_stake_address(wallet_id: int, stake_address: str) -> None:
    """Persist a resolved stake key onto a wallet row
    (ABCT-CARDANO-ACCOUNT-LEVEL-20260919). A payment address's stake
    credential never changes, so once set this never needs re-resolving --
    callers should check the existing value before calling this again."""
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "UPDATE wallets SET stake_address = ? WHERE id = ?",
            (stake_address, wallet_id),
        )
        await db.commit()


async def get_stake_group_wallets(user_id: int, stake_address: str) -> list:
    """Every wallet row sharing this (user, stake_address) group, ordered so
    the FIRST row is always the canonical one (ABCT-CARDANO-ACCOUNT-LEVEL-
    20260919):
      1. Prefer a user-set label over an auto-generated one
         (label_is_auto = 0 first).
      2. Among those, prefer a row that actually HAS a label over one with
         none -- most of a stake group's addresses are typically unlabeled
         (only the one the user actually named, e.g. "vault", isn't), and
         label_is_auto = 0 alone doesn't distinguish "unlabeled" from
         "user-labeled".
      3. Tiebreak on lowest wallet id.
    """
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("""
            SELECT * FROM wallets
            WHERE user_id = ? AND stake_address = ?
            ORDER BY
                COALESCE(label_is_auto, 0) ASC,
                CASE WHEN label IS NOT NULL AND label != '' THEN 0 ELSE 1 END ASC,
                id ASC
