...
class BinanceUSService:
    ...
    def is_configured(self) -> bool:
        """Check if Binance.US API is properly configured (sync, checks cached keys)."""
    ...
    async def ensure_configured(self) -> bool:
        ...
        if not self.api_key or not self.api_secret:
            try:
                ...
                setting = await get_api_setting("binance_us")
                if setting:
                ...
                logger.debug(f"Could not load Binance.US keys from DB: {e}")
