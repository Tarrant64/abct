...
class TransactionHistoryService:
    ...
    async def _fetch_bitcoin_transactions(self, address: str, limit: int) -> List[dict]:
        ...
        # Bitcoin service would need transaction history method
        logger.info("Bitcoin transaction history not yet implemented")
        return []
    ...
    async def _normalize_bitcoin_transaction(self, tx: dict, wallet_address: str) -> dict:
        """Normalize Bitcoin transaction."""
        # Placeholder - implement when Bitcoin tx API is available
        return {
            'blockchain': 'bitcoin',
            'tx_hash': tx.get('hash', ''),
            'tx_time': datetime.utcnow(),
