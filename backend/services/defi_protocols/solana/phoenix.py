"""Phoenix adapter - open order position detection."""

import logging
from typing import List
from services.defi_protocols.base_adapter import DetectionMethod, PositionType, ProtocolPosition
from services.defi_protocols.solana.base_solana_adapter import BaseSolanaAdapter, get_helius_rpc_url
from services.http_client import get_client

logger = logging.getLogger(__name__)

PHOENIX_PROGRAM = "PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY"
# Phoenix trader state account size
PHOENIX_SEAT_SIZE = 120


class PhoenixAdapter(BaseSolanaAdapter):
    PROTOCOL_NAME = "Phoenix"
    SUPPORTED_CHAINS = ["solana"]
    DETECTION_METHOD = DetectionMethod.PROGRAM_ACCOUNT
    PROTOCOL_URL = "https://ellipsis.markets"

    async def detect_positions(self, address: str, chain: str = None) -> List[ProtocolPosition]:
        """Detect Phoenix open order / seat positions."""
        rpc_url = await get_helius_rpc_url()
        if not rpc_url:
            return []

        try:
            client = get_client("helius", timeout=30.0)
            response = await client.post(rpc_url, json={
                "jsonrpc": "2.0",
                "id": 1,
                "method": "getProgramAccounts",
                "params": [
                    PHOENIX_PROGRAM,
                    {
                        "encoding": "base64",
                        "filters": [
                            {"dataSize": PHOENIX_SEAT_SIZE},
                            # Trader/owner at offset 8
                            {"memcmp": {"offset": 8, "bytes": address}},
                        ]
                    }
                ]
            })

            if response.status_code != 200:
                return []

            data = response.json()
            accounts = data.get('result', [])

            if not accounts:
                return []

            positions = []
            for account in accounts:
                pubkey = account.get('pubkey', '')
                positions.append(ProtocolPosition(
                    protocol="Phoenix",
                    chain="solana",
                    position_type=PositionType.PERPETUALS,
                    token_symbol="PHX",
                    token_name="Phoenix Open Orders",
                    amount=1.0,
                    contract_address=pubkey,
                    extra={
                        "program_id": PHOENIX_PROGRAM,
                        "seat_account": pubkey,
                        "note": "Open order details require on-chain data decoding",
                    },
                ))

            return positions

        except Exception as e:
            logger.error(f"Error detecting Phoenix positions for {address[:20]}...: {e}")
            return []


from services.defi_protocols.registry import protocol_registry
protocol_registry.register(PhoenixAdapter())
